...
class SecurityChecker:
    ...
    def generate_reports(self, test_results: list):
        ...
        try:
            ...
            # Catch missing dependency gracefully, log warning, and create a structured text file error placeholder
            import logging
            logging.getLogger("redactai.security_checker").warning(
                f"ReportLab package is unavailable. Skipping PDF security report compilation: {e}"
            )
            try:
                with open(err_txt_path, "w") as f:
                    f.write("PDF report generation is unavailable because the optional 'reportlab' package is not installed.")
            except Exception:
                pass
